...
The viewer is **five** files, not three: `index.html`, `app.js`, `style.css`, `tour.js`, `selftest.html`. Mirror all five.

## Deployment

> **⚠️ Do not run `prepare_github_pages.py` for code-only viewer edits.** Its `copy_viewer()` does `shutil.rmtree(docs/brain-viewer)` and then `copytree` from the **gitignored** `brain-gan-viewer/viewer/` — it deletes ~1261 tracked files (25 MB of data — `du` reports ~173 MB because 1260 small files each occupy a full cluster on this volume) and repopulates from an untracked source. It is safe only while the two trees are identical. For editing them, change the tracked copy under `docs/brain-viewer/`, then `cp` the five files into `brain-gan-viewer/viewer/`, and confirm `git status --short` shows only those paths.
...
There is no WebGL in the agent environment, so **no change to the viewer can be visually verified from here**. [docs/brain-viewer/selftest.html](docs/brain-viewer/selftest.html) exists to narrow that gap: it loads `index.html` in a same-origin iframe, steps it through five viewports (1920/1440/1024/768/390), and asserts geometry, drawer behaviour, metrics wiring, tumour-control consistency and the tour against the real DOM. Open it after **any** viewer edit — `cd docs && python3 -m http.server 8080`, then `/brain-viewer/selftest.html`. It auto-runs and puts the failure count in the tab title.

- The stage iframe is **deliberately visible and CSS-scaled**, not parked off-screen: an off-screen iframe gets its `requestAnimationFrame` throttled, which stalls the tour-advance assertion. `transform: scale()` does not affect `innerWidth`, so media queries still see the true width.
- **It cannot judge appearance.** Depth ordering through the translucent shell while orbiting, and z-fighting where the tumour meets the slice quad, are not detectable from the DOM and remain a human check. The page says so itself — do not let a green run stand in for looking at it.
- The tumour assertion compares a `HEAD` on `tumor_surface.glb` against `#tumor-group`'s visibility, so it passes both before and after the mask run rather than hard-coding either state.
- Cancellation is exercised through the Skip button, not a synthetic canvas gesture: `dispatchEvent` can never produce `isTrusted === true`, which is exactly what the tour's bail guard requires.
...
- **The mask stack is on the DISPLAY grid, not the native one.** `pred_mask_vol` and `u_evolved_native` are native (e.g. 240²); the exported slice PNGs are 256². `export_mask_stack` resamples with the same `INTER_NEAREST → rot90 → threshold` idiom the growth animation uses. Skip that and the mesh is squeezed into a corner of the brain. This is the same family as the `spacing_mm` vs `pixel_spacing_mm` rule above — **display-grid voxel counts run ~14% above native and must never be reconciled with `metrics.json`.** metrics.json owns physical mL; the mesh owns viewer geometry.
- **Build-time volume gate.** `build_tumor_mesh()` prints mesh volume against `n_voxels * voxel_mm3` and flags >5%. This is a *marching-cubes fidelity* check in display-grid units — it asks "did the mesh preserve the mask's shape", **not** "does it equal metrics.json". If it exceeds 5%, lower `--tumor_field_sigma`. Decimation can leave a stray non-manifold edge; the divergence-theorem volume stays accurate and is reported with a trust label rather than withheld.
...
- **`--tumor_only` is the only supported way to add a tumour to a built variant.** A full `generate_brain.py` run rewrites `brain_surface.glb` and 384 slice PNGs — ~8 MB and 385 files of churn per variant for geometry that did not change.
